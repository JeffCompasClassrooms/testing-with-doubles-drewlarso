...
def describe_MyDB():
    ...
    def describe_saveStrings():
        def it_saves_the_given_array_to_a_file(mocker):
            ...
            mock_open.assert_called_once_with("mydatabase.db", "wb")
            mock_dump.assert_called_once_with(["hello", "world"], mock_open.return_value)

    def describe_saveString():
        def it_appends_string_element_to_existing_database(mocker):
            mock_isfile = mocker.patch("os.path.isfile", return_value=True)
            mock_open = mocker.patch("builtins.open", mocker.mock_open())
            mock_load_strings = mocker.patch("mydb.MyDB.loadStrings", return_value=["hello", "world"])
            ...
            mock_save_strings.assert_called_once_with(["hello", "world", "python"])
